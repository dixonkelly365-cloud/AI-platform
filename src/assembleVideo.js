...
async function assemble() {
  ...
  for (const scene of storyboard.scenes) {
    ...
    // Deliberate pacing, not uniform mechanical zoom: the hook (scene 1)
    // and the twist moment (roughly 2/3 through, where the story's real
    // reveal tends to land) get a faster, more dramatic push — genuine
    // emphasis at the beats that matter. Setup scenes hold steadier. The
    // final scene eases slightly, letting the loop-back line land instead
    // of rushing off the video.
    const progressFraction = totalScenes > 1 ? sceneIndex / (totalScenes - 1) : 0;
    const isHook = sceneIndex === 0;
    const isTwistZone = progressFraction >= 0.55 && progressFraction <= 0.8;
    const isClosing = sceneIndex === totalScenes - 1;

    let zoomRate = 0.0012; // steady default, used for setup/middle scenes
    if (isHook || isTwistZone) zoomRate = 0.0022; // faster, more dramatic push at key beats
    if (isClosing) zoomRate = 0.0008; // calmer, lets the closing line breathe

    function kenBurnsClip(photoPath, duration, outPath, zoomDirIn) {
      const totalFrames = Math.round(duration * FPS);
      const zoomExpr = zoomDirIn
        ? `min(zoom+${zoomRate},1.3)`
        : `if(lte(zoom,1.0),1.3,max(1.0,zoom-${zoomRate}))`;
      const filter =
